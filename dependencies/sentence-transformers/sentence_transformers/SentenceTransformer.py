...
class SentenceTransformer(nn.Sequential):
    ...
    def encode(self, sentences: Union[str, List[str], List[int]],
        ...
        for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
            ...
            ## Ensure token update
            some_pass = False
            if self.tokenizer.tokenize('This is a IDancienthistoryID')[-1] == 'IDancienthistoryID' :
                some_pass = True
    
            if self.tokenizer.tokenize( 'This is a IDcolégiomilitarID'     )[-1] == 'IDcolégiomilitarID'    :
                some_pass = True
            ## Not going to require this - use for validation
            ## assert some_pass
